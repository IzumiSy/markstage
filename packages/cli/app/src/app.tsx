...
function useGroupedEntries(previewEntries: PreviewEntry[]) {
  return useMemo(() => {
    const visible = previewEntries.filter((e) => !e.frontmatter.hidden);
    ...
    return [...groupMap.values()].toSorted((a, b) => a.order - b.order);
  }, [previewEntries]);
}
  ...
  return (
    ...
    >
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        >
          <TableOfContents containerRef={scrollRef} contentKey={entry.name} />
        </div>
      </div>
    </div>
  );
